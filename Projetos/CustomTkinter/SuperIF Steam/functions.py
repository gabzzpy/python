...
def AdicionarJogos(nome,genero,desenvolvedor,data,sinopse):
    ...
    if nome.strip() == '' or genero.strip() == '' or desenvolvedor.strip() == ''or data.strip() == ''or sinopse.strip() == '':
        return(False,'Os valores não podem ser em branco')
    if " " in data:
        return (False, 'A data não pode conter espaços')
    if data:
...
def Edit_or_remove(edit_or_remove='',what='',where='',change=''):
    ...
    if edit_or_remove == '' or what == ''or where == '':
        return (False,'Existe alguma informação faltando')
    if edit_or_remove.strip() == '' or what.strip() == '' or where.strip() == '' or change.strip() == '':
        return(False,'Os valores não podem ser em branco')
    try:
