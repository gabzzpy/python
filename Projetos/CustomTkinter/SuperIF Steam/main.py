...
def AdicionarJogo():
    ...
    if check[0]:
        por_img.configure(text='Selecione\na imagem')
        Apagar()
        apagar_fotos_pasta("C:/Users/gabmo/OneDrive/Área de Trabalho/SuperIF Steam/ImagensJogos")
        GoAddgames()
        Posicionar_Erro(check[1])
        voltar.place(x=60,y=120)
...
def MostrarJogos():
    ...
    for widget in lista_jogosFrame.winfo_children():
        widget.destroy()
...
def EditorRemove():
    ...
    if edit == 'editar':
        ...
        erro = Edit_or_remove(what=what,edit_or_remove=edit,where=where,change='remover')
...
def GoDados():
    ...
    logout_button.place(x=160,y=580)
...
logout_button =  CTkButton(detalhe_cinza_frame2,fg_color=Vermelho_Neon,text_color=Preto,text='Logout',font=('Neue Machina',15,'bold'),corner_radius=25,height=40,width=180,command=Logout)
...
def MostrarJogosBiblio():
    ...
    for widget in biblioteca_jogosFrame.winfo_children():
        widget.destroy()
...
def PassarImagens():
    ...
    if contador == 10:
        contador = 0
        imagem_tk = Organizar_imagens(f"Imagens/Paisagem{randint(1,6)}.png",647,380)
...
def Posicionar_Sinopse(id):
    ...
    for row in rows:
        if row[0] == id:
            sinopse = row[5]
            text = f'O usuário {user_in_moment[4]} adicionou o jogo {row[1]}'
    print(sinopse)
    print(id)
...
GameSelect = CTkFrame(janela,width=900,height=600,fg_color=Verde_Claro2,corner_radius=5)
...
botao_GameSelect_sair = CTkButton(GameSelect,fg_color=Vermelho_Neon,font=('DM Sans',20,'bold'),text_color=Verde_Escuro,text='Sair',width=150,command=SairErro3,hover_color=Verde_Claro1,corner_radius=0)
...
label2 = CTkLabel(GameSelect,fg_color=Verde_Claro2,text_color=Branco,font=('DM Sans',14,'bold'),anchor=CENTER,width=100,corner_radius=0)
...
botao_mudar_horas = CTkButton(GameSelect,fg_color=Verde_Neon,font=('DM Sans',20,'bold'),text_color=Verde_Escuro,text='Mudar Horas Jogadas',width=150,hover_color=Verde_Claro1,corner_radius=0)
